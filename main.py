...
import time
import machine  # Importa el mÃ³dulo machine para reiniciar el sistema
...
archivo_indice = "/ultimo_indice.txt"  # Para controlar el envÃ­o de datos a MongoDB

# ConfiguraciÃ³n de UART para el sensor y GPS
uart_sensor = UART(0, baudrate=4800, tx=Pin(0), rx=Pin(1))
modulo_gps = UART(1, baudrate=9600, tx=Pin(4), rx=Pin(5))

# InicializaciÃ³n de la librerÃ­a GPS
Zona_Horaria = -5
...
timer = Timer()
captura_activa = False  # Variable de estado para controlar si la captura estÃ¡ activa
server_socket = None
...
# FunciÃ³n para calcular el CRC16
...
# FunciÃ³n para enviar comandos Modbus al sensor
...
# FunciÃ³n para calibrar el sensor
# FunciÃ³n para calibrar el sensor
def calibrar_sensor(factor_a_nitrogeno=None, offset_nitrogeno=None, factor_a_fosforo=None, offset_fosforo=None, factor_a_potasio=None, offset_potasio=None):
    # Comandos Modbus para cada variable si los valores estÃ¡n presentes
    if factor_a_nitrogeno is not None:
        print(f"Calibrando Factor A de NitrÃ³geno: {factor_a_nitrogeno}")
    ...
    if offset_nitrogeno is not None:
        print(f"Calibrando Offset de NitrÃ³geno: {offset_nitrogeno}")
    ...
    if factor_a_fosforo is not None:
        print(f"Calibrando Factor A de FÃ³sforo: {factor_a_fosforo}")
    ...
    if offset_fosforo is not None:
        print(f"Calibrando Offset de FÃ³sforo: {offset_fosforo}")
...
def verificar_archivo_csv():
    if not archivo_existe(archivo_csv):
        with open(archivo_csv, "w") as archivo:
            archivo.write("Fecha,Hora,Latitud,Longitud,Humedad,Temperatura,Conductividad,pH,NitrÃ³geno,FÃ³sforo,Potasio\n")

# Verificar si el archivo de credenciales existe
def verificar_archivo_wifi():
    if not archivo_existe(archivo_wifi):
        with open(archivo_wifi, "w") as archivo:
            archivo.write("")  # Crear archivo vacÃ­o si no existe
...
# FunciÃ³n para editar credenciales Wi-Fi
...
# FunciÃ³n para conectar a Wi-Fi usando las credenciales guardadas con lÃ­mite de tiempo
def conectar_wifi(ssid, password, tiempo_limite=10):
    ...
    while not wlan.isconnected():
        if time.time() - tiempo_inicio > tiempo_limite:  # Verificar si se excediÃ³ el tiempo lÃ­mite
            print(f"No se pudo conectar a {ssid} en el tiempo lÃ­mite de {tiempo_limite} segundos.")
            wlan.active(False)  # Desactivar el Wi-Fi si no se logra la conexiÃ³n
            return False
...
# FunciÃ³n para borrar los archivos CSV y de Ã­ndice
def borrar_archivos():
    try:
        # Borrar archivo CSV
        ...
        # Borrar archivo del Ãºltimo Ã­ndice
...
# FunciÃ³n para apagar el AP y cerrar el servidor
...
#         # Si el servidor estÃ¡ activo, cerrarlo
#         if server_socket:
#             server_socket.close()
#             print("Servidor cerrado.")
#             server_socket = None  # Resetear la variable para indicar que el servidor estÃ¡ cerrado
#             
#             # Agregar un pequeÃ±o retraso para asegurarse de que el puerto se libera
#             time.sleep(1)
...
def activar_punto_de_acceso(ap):
    ...
    if not ap.active():
        ...
        if server_socket is None:
            ...
            try:
                time.sleep(1)  # Espera un momento para asegurarte de que el puerto estÃ© libre
...
def escanear_y_conectar_redes(tiempo_limite_por_red=10):
    ...
    with open(archivo_wifi, "r") as archivo:
        ...
        conexion_exitosa = False  # Variable para rastrear si se conectÃ³ a alguna red
    ...
    if not conexion_exitosa:
        ...
        activar_punto_de_acceso(ap)  # Volver a activar el AP si no se logra la conexiÃ³n
    
    return conexion_exitosa  # Devuelve True si la conexiÃ³n fue exitosa, de lo contrario False
...
def enviar_datos_a_mongodb(fecha, hora, latitud, longitud, sensor_data):
    ...
    payload = {
        ...
        "document": {
            "Fecha": fecha,
            "Hora": hora,
            "Latitud": float(latitud),  # Asegurar que sea nÃºmero
            "Longitud": float(longitud),  # Asegurar que sea nÃºmero
            "Humedad": sensor_data['Humedad'],
            "Temperatura": sensor_data['Temperatura'],
            "Conductividad": sensor_data['Conductividad'],
            "pH": sensor_data['pH'],
            "Nitrogeno": sensor_data['NitrÃ³geno'],  # Valor validado de NitrÃ³geno
            "Fosforo": sensor_data['FÃ³sforo'],      # Valor validado de FÃ³sforo
            "Potasio": sensor_data['Potasio']       # Valor validado de Potasio
        }
    }

    # Mostrar el payload y su tamaÃ±o antes de enviarlo
    print("Enviando el siguiente payload a MongoDB:")
    print(ujson.dumps(payload))  # Mostrar el JSON con formato
    print("TamaÃ±o del payload:", len(ujson.dumps(payload)))
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"Error al enviar datos. CÃ³digo de estado: {response.status_code}")
            return False
    except Exception as e:
        print(f"Error al enviar los datos a MongoDB: {e}")
        return False

# Subir datos del CSV empezando desde el Ãºltimo Ã­ndice
# Subir datos del CSV empezando desde el Ãºltimo Ã­ndice
def subir_datos_csv():
    ...
    if archivo_existe(archivo_csv):
        ...
        for indice, linea in enumerate(lineas[ultimo_indice:], start=ultimo_indice):
            ...
            sensor_data = {
                ...
                "pH": float(fila[7]),
                "NitrÃ³geno": float(fila[8]),
                "FÃ³sforo": float(fila[9]),
    ...
    # AquÃ­ se hace el reinicio cuando todos los datos han sido enviados
...
# FunciÃ³n para leer datos del sensor
def read_sensor():
    print("TransmisiÃ³n habilitada, enviando datos al sensor")
    uart_sensor.write(queryData)
    time.sleep(0.02)
    print("RecepciÃ³n habilitada")
    time.sleep(0.6)
...
# FunciÃ³n para leer datos del GPS
def get_gps_data():
...
# FunciÃ³n para convertir los datos del GPS en formato adecuado
...
# FunciÃ³n para manejar la captura continua de datos
...
def guardar_datos_csv(fecha, hora, latitud, longitud, sensor_data):
    with open(archivo_csv, "a") as archivo:
        ...
        archivo.write(f"{sensor_data['Conductividad']},{sensor_data['pH']},{sensor_data['NitrÃ³geno']},")
        archivo.write(f"{sensor_data['FÃ³sforo']},{sensor_data['Potasio']}\n")

# FunciÃ³n para iniciar el punto de acceso
def iniciar_punto_de_acceso():
    ap = network.WLAN(network.AP_IF)
    ap.config(essid="PicoW_Setup", password="12345678")
    ap.active(True)
    print("Punto de acceso iniciado. ConÃ©ctese a la red 'PicoW_Setup' con la contraseÃ±a '12345678'.")
    print("DirecciÃ³n IP del punto de acceso:", ap.ifconfig()[0])
    return ap


# FunciÃ³n para manejar las solicitudes HTTP
...
<body>
    <div class="container">
        ...
        <h2>Borrar Archivos CSV e Ãndice</h2>
    ...
    while True:
        try:
            ...
            if "GET / " in request:
                ...
                cl.send('HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n')
                cl.send(ujson.dumps({"message": "CalibraciÃ³n enviada al sensor correctamente"}).encode())
